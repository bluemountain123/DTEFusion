...
class F_NSWT(nn.Module):
    ...
    def _transformer(self, low, high):
        return torch.cat([low, high], dim=1)  # Output shape: [N, 4C, H, W].
    ...
    def forward(self, x):
        ...
        batch, C, H, W = x.shape
        arr = x[0].detach().cpu().numpy()  # The current implementation assumes batch size 1.
        all_cA, all_high = [], []
...
class MultiSpectralAttentionLayer(nn.Module):
    """Multi-spectral attention layer."""
