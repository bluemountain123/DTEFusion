...
# Set a fixed random seed for reproducible data crops and splits.
...
class VIFDataset(Dataset):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self.labels = labels
        self.image_size = (224, 224)  # Use a fixed crop size.
    ...
    def __getitem__(self, index: int) -> tuple[Tensor, Tensor]:
        ...
        # Ensure each image is large enough for the configured crop size.
        ...
        # Pick one random crop region and apply it to both modalities.
        box = self.random_area(vi.size)
        with torch.no_grad():
...
def read_and_split(root: str, ratios: Tuple[float, float] | Tuple[float, float, float] | None):
    ...
    assert sum(ratios) == 1, 'Dataset split ratios must sum to 1.'
...
def parse_directory(root: str) -> List[Dict[str, str]]:
    ...
    assert 'vi' in classes, 'Missing visible-light dataset directory: vi'
    assert 'ir' in classes, 'Missing infrared dataset directory: ir'
