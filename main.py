...
def tensor_to_image(img: Tensor, size: torch.Size) -> Image.Image:
    ...
    # Crop away padding and remove the batch dimension.
...
class Color:
    ...
    def YCbCr_to_RGB(self, y: Tensor, cb: Tensor, cr: Tensor) -> Tensor:
        """
        Convert Y in [0, 1] plus Cb and Cr in [-0.5, 0.5] back to RGB in [0, 1].
        """
        r = y + 2 * cr * (1 - self.kr)
        b = y + 2 * cb * (1 - self.kb)
        g = (y - self.kr * r - self.kb * b) / self.kg
        
        # Concatenate and clamp to [0, 1] to avoid floating-point overflow.
...
def test(config: TrainConfig, path: str, device: torch.device):
    ...
    save_path = save_path.parent / (save_path.name + '-Test') / model_name
    print(f'{save_path=}')
    ...
    # Create the color conversion helper.
    color_util = Color()
    ...
    with torch.no_grad():
        for (vi_color_raw, vi_gray_raw, ir_raw, filename) in load_data(path):
            ...
            # 1. Prepare model inputs in the [-1, 1] range.
            inputs, size = pad(vi_gray, ir, config.model.params['patch_size'])
            
            # 2. Run the model and convert the fused luminance to [0, 1].
            output: FuseOutput = model(inputs)
            y_fused = (output.fusion + 1.0) / 2.0

            # 3. Prepare the visible-light color image for chroma transfer.
            vi_color_normalized = (vi_color + 1.0) / 2.0
            
            # 4. Pad the normalized color image to match the fused tensor size.
            H, W = size
            ...
            # 5. Extract chroma channels (Cb, Cr).
            _, cb, cr = color_util.RGB_to_YCbCr(vi_color_padded)

            # 6. Combine the fused luminance with original chroma and convert to RGB.
            final_rgb_fusion = color_util.YCbCr_to_RGB(y_fused, cb, cr)

            # 7. Save the fused image.
...
def load_data(dir: str) -> Iterator[tuple[Tensor, Tensor, Tensor, str]]:
    if 'MSRS' in dir:
        ...
        for item in labels:
            vi_color_pil = Image.open(item['vi']).convert('RGB')
            vi_color = PIL_to_Tensor(vi_color_pil).unsqueeze(0)
            vi_gray = PIL_to_Tensor(vi_color_pil.convert('L')).unsqueeze(0)
            ir = PIL_to_Tensor(Image.open(item['ir']).convert('L')).unsqueeze(0)
            filename = os.path.basename(item['vi'])
            yield vi_color, vi_gray, ir, filename
    elif 'Road' in dir:
        road = Path(dir)
        ...
        M3FD = Path(dir)
        with open(M3FD.joinpath('labels.txt'), 'r') as fp:
            filenames = fp.read().splitlines()
            for filename in filenames:
                vi_file = M3FD.joinpath('vi', filename)
                ir_file = M3FD.joinpath('ir', filename)
...
if __name__ == '__main__':
    ...
    config_path = "/root/autodl-tmp/DTEFusion-main/config/model2.yaml"
    config = load_config(config_path)

    # Path for saving results
    save_loss_plot_path = "/root/autodl-tmp/DTEFusion-main/result123"
    ...
    if args.evolve:
        ...
        # Test
        config.model.pretrained ="/root/autodl-tmp/code/WOA_result2/evolve.pth"
